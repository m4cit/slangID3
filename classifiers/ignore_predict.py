...
tokenData = pd.read_csv(r'./classifiers/modifiers/data/processed_token_data.csv')
trainTk, testTk = train_test_split(tokenData, test_size=0.1, random_state=42)
trainTk_x = [x for x in trainTk['phrase']]
trainTk_y = [x for x in trainTk['type']]
testTk_x = [x for x in testTk['phrase']]
testTk_y = [x for x in testTk['type']]

vectorizerTk = CountVectorizer(ngram_range=(1,1))
vect_trainTk_x = vectorizerTk.fit_transform(trainTk_x)
vect_testTk_x = vectorizerTk.transform(testTk_x)
...
def train_svm():
    ...
    clf_svm_tk.fit(vect_trainTk_x, trainTk_y)
    progressbar.update()
...
def train_log():
    ...
    clf_log_tk.fit(vect_trainTk_x, trainTk_y)
    progressbar.update()
    # saving trained models
...
def train_dt():
    ...
    clf_dt_tk.fit(vect_trainTk_x, trainTk_y)
    progressbar.update()
    # saving trained models
...
def train_nbg():
    ...
    clf_nbg_tk.fit(vect_trainTk_x.toarray(), trainTk_y)
    progressbar.update()
    # saving trained models
...
def train_nbm():
    ...
    clf_nbm_tk.fit(vect_trainTk_x, trainTk_y)
    progressbar.update()
    # saving trained models
...
def predictor_svm(input, input2):
    try:
        ...
        with open('./classifiers/models/pd_linear_svm_classifier.pkl','rb') as f:
            ...
            out1 = "Linear SVM: \n  Phrase:\t      " + input[0] + "  Prediction:\t " + loaded_clf.predict(vectorizer.transform(input))[0] + " | mean accuracy:\t " + str(loaded_clf.score(vect_test_x, test_y)) + "\n   |"
        ...
        with open('./classifiers/models/pd_linear_svm_classifier_tk.pkl','rb') as f2:
            ...
            for w in range(len(words)):
                index = "  [" + str(w+1) + "]"
                print("\n\t\tWord:\t\t", words[w].replace('\n', ''), "\n\t\tPrediction:\t", loaded_clf_tk.predict(vectorizerTk.transform(words)[w])[0])
                print("\t\tmean accuracy:\t", loaded_clf_tk.score(vect_testTk_x, testTk_y))
                out2 += index + " Word:\t   " + str(words[w].replace('\n', '')) + "\n  Prediction:\t " + str(loaded_clf_tk.predict(vectorizerTk.transform(words)[w])[0]) + " | mean accuracy:\t " + str(loaded_clf_tk.score(vect_testTk_x, testTk_y)) + "\n"
            end = "\n"
...
def predictor_naive_g(input, input2):
    try:
        ...
        with open('./classifiers/models/pd_gaussian_nb_classifier.pkl','rb') as f:
            ...
            out1 = "Naive Bayes (Gaussian): \n  Phrase:\t      " + input[0] + "  Prediction:\t " + loaded_clf.predict(vectorizer.transform(input).toarray())[0] + " | mean accuracy:\t " + str(loaded_clf.score(vect_test_x.toarray(), test_y)) + "\n   |"
        ...
        with open('./classifiers/models/pd_gaussian_nb_classifier_tk.pkl','rb') as f2:
            ...
            for w in range(len(words)):
                index = "  [" + str(w+1) + "]"
                print("\n\t\tWord:\t\t", words[w].replace('\n', ''), "\n\t\tPrediction:\t", loaded_clf_tk.predict(vectorizerTk.transform(words)[w].toarray())[0])
                print("\t\tmean accuracy:\t", loaded_clf_tk.score(vect_testTk_x.toarray(), testTk_y))
                out2 += index + " Word:\t   " + str(words[w].replace('\n', '')) + "\n  Prediction:\t " + str(loaded_clf_tk.predict(vectorizerTk.transform(words)[w].toarray())[0]) + " | mean accuracy:\t " + str(loaded_clf_tk.score(vect_testTk_x.toarray(), testTk_y)) + "\n"
            end = "\n"
            return out1, out2, end
    except FileNotFoundError:
        print("Please train the model(s) first!")
