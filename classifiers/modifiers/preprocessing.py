    ...
    with open(file, 'r', encoding='UTF-8', newline='') as rawInfile:
        with open(filepath+"processed_"+filename, 'w', encoding='UTF-8', newline='') as tempOutfile:
            ...
            for line in linesRaw:
                tempOutfile.write(line.replace('<eex>', '').replace('<en>', '').replace('<eadj>', '').replace('<mwexn>', '').replace('<shmex>', '').replace('<shnpl>', '').replace('<sha>', '').replace('<adj>', '').replace('<v>', '').replace('<mwn>', '').replace('<n>', '').replace('<npl>', '').replace('<pex>', '').replace('[', '').replace(']', '').replace('-', ' '))    
                progressbar.update()
            progressbar.close()
